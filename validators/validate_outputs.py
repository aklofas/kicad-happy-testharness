...
def main():
    ...
    parser.add_argument("--results-dir", default=str(HARNESS_DIR / "results" / "outputs" / "schematic"),
                        help="Directory containing JSON output files")
    parser.add_argument("--manifests-dir", default=str(HARNESS_DIR / "results" / "manifests"),
    ...
    if not schematics_list.exists():
        print(f"Error: {schematics_list} not found. Run discover.sh first.")
        sys.exit(1)
